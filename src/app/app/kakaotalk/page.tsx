  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10">
      <section className="bg-slate-800/40 rounded-3xl p-8 shadow-sm border border-slate-700/50 flex flex-col md:flex-row gap-8 items-start md:items-center">
        <div className="w-24 h-24 bg-yellow-400 text-white rounded-3xl flex items-center justify-center text-5xl font-bold flex-shrink-0">
          K
        </div>
        <div className="flex-1">
          <div className="inline-block bg-yellow-100 text-yellow-800 text-xs font-bold px-3 py-1 rounded-full mb-3">국민 메신저</div>
          <h1 className="text-3xl font-bold text-white mb-2">카카오톡 PC버전</h1>
      ...
      <section className="bg-slate-800/60 rounded-3xl p-8 text-center border border-slate-700/50">
        <h2 className="text-2xl font-bold text-white mb-4">대한민국 국민 필수 PC 메신저 다운로드</h2>
        <p className="text-slate-300 mb-8">모바일과 100% 동기화되는 카카오톡 PC버전을 지금 바로 다운로드하세요.</p>
        
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a href="https://www.kakaocorp.com/page/service/service/KakaoTalk" target="_blank" rel="nofollow noopener" className="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-4 px-8 rounded-xl shadow-lg transition transform hover:-translate-y-1 flex items-center justify-center gap-2">
      ...
      <section className="bg-slate-800/40 rounded-3xl p-8 shadow-sm border border-slate-700/50 prose prose-blue max-w-none">
        <h2 className="text-2xl font-bold text-white mb-4 mt-0">직장인을 위한 카카오톡 엑셀(몰컴) 모드 설정법</h2>
